...
from audit_aggregators import (
    build_control_snapshot_response as _build_control_snapshot_response_impl,
    build_latest_scheduler_command as _build_latest_scheduler_command_impl,
    build_scheduler_command_impact_detail as _build_scheduler_command_impact_detail_impl,
)
...
from strategy_alerts import (
    ...
    classify_strategy_auto_dispatch_alert_kind as _classify_strategy_auto_dispatch_alert_kind_impl,
    clear_strategy_auto_dispatch_alerts as _clear_strategy_auto_dispatch_alerts_impl,
    clear_strategy_exchange_rejected_alerts as _clear_strategy_exchange_rejected_alerts_impl,
    clear_strategy_exchange_rejection_guard_alerts as _clear_strategy_exchange_rejection_guard_alerts_impl,
    clear_strategy_live_stop_loss_alerts as _clear_strategy_live_stop_loss_alerts_impl,
    clear_strategy_manual_execution_alerts as _clear_strategy_manual_execution_alerts_impl,
    clear_strategy_position_drift_alerts as _clear_strategy_position_drift_alerts_impl,
    clear_strategy_stale_order_alerts as _clear_strategy_stale_order_alerts_impl,
    derive_sub_block_code_from_detail as _derive_sub_block_code_from_detail_impl,
    has_active_strategy_auto_dispatch_alert as _has_active_strategy_auto_dispatch_alert_impl,
    has_active_strategy_exchange_rejection_guard_alert as _has_active_strategy_exchange_rejection_guard_alert_impl,
    has_active_strategy_live_stop_loss_alert as _has_active_strategy_live_stop_loss_alert_impl,
    has_active_strategy_position_drift_alert as _has_active_strategy_position_drift_alert_impl,
    has_active_strategy_stale_order_alert as _has_active_strategy_stale_order_alert_impl,
    queue_strategy_issue_review_locked as _queue_strategy_issue_review_locked_impl,
    record_strategy_auto_dispatch_issue as _record_strategy_auto_dispatch_issue_impl,
    record_strategy_live_stop_loss_issue as _record_strategy_live_stop_loss_issue_impl,
    record_strategy_manual_execution_issue as _record_strategy_manual_execution_issue_impl,
...
def _build_scheduler_command_impact_detail(payload: Dict[str, Any]) -> Optional[str]:
    return _build_scheduler_command_impact_detail_impl(payload)


def _build_latest_scheduler_command(audit_events: List[ExecutionEvent]) -> Optional[LatestSchedulerCommand]:
    return _build_latest_scheduler_command_impl(
        audit_events,
        non_empty_string=_non_empty_string,
        non_empty_string_list=_non_empty_string_list,
        single_or_none=_single_or_none,
    )


def _build_control_snapshot_response() -> ControlSnapshot:
    return _build_control_snapshot_response_impl(
        repo=repo,
        build_strategy_runtime_worker_health=_build_strategy_runtime_worker_health,
        collect_dynamic_auto_dispatch_blocked_contexts=_collect_dynamic_auto_dispatch_blocked_contexts,
        resolve_strategy_primary_market=_resolve_strategy_primary_market,
        ...
        build_strategy_active_order_summary=_build_strategy_active_order_summary,
        build_strategy_position_alignment_summary=_build_strategy_position_alignment_summary,
        build_execution_health_top_issue_context=_build_execution_health_top_issue_context,
        build_latest_scheduler_command=_build_latest_scheduler_command,
    )
...
def build_backtest_payload(strategy: Any, data_range: str, timeframe: str) -> Optional[Dict[str, Any]]:
    """Round 118 thin re-export — delegates to :func:`backtest_engine.build_backtest_payload`.

    The backtest payload builder is the only callable in this set that needs
    runtime state (``repo``, ``market_data``, the strategy → primary-market
    resolver), so the dependencies are passed through as keyword arguments
    rather than copied into the engine module.  ``repo.backtest_runner = ...``
    further down still expects a callable with the original
    ``(strategy, data_range, timeframe)`` signature, so the wrapper preserves
    that shape while keeping the implementation in :mod:`backtest_engine`.
    """

    return backtest_engine.build_backtest_payload(
        strategy,
        data_range,
        timeframe,
        repo=repo,
...
def _build_sample_validation_backtest_payload(
    ...
    """Round 118 thin re-export — see :func:`backtest_engine.build_sample_validation_backtest_payload`."""

    return backtest_engine.build_sample_validation_backtest_payload(
        data_range,
        timeframe,
        prefer_finer_timeframe=prefer_finer_timeframe,
    )


def _resolve_history_source_recommendation(context: Dict[str, Any]) -> Dict[str, str]:
    """Round 118 thin re-export — see :func:`backtest_engine.resolve_history_source_recommendation`."""

    return backtest_engine.resolve_history_source_recommendation(context)


def _max_coverable_days_for_timeframe(timeframe: str) -> int:
    """Round 118 thin re-export — see :func:`backtest_engine.max_coverable_days_for_timeframe`."""

    return backtest_engine.max_coverable_days_for_timeframe(timeframe)


def _build_coverable_backtest_range(data_range: str, timeframe: str) -> str:
    """Round 118 thin re-export — see :func:`backtest_engine.build_coverable_backtest_range`."""

    return backtest_engine.build_coverable_backtest_range(data_range, timeframe)
...
def _build_available_history_data_range_from_bounds(
    ...
    """Round 118 thin re-export — see :func:`backtest_engine.build_available_history_data_range_from_bounds`."""

    return backtest_engine.build_available_history_data_range_from_bounds(
        start_iso,
        end_iso,
        timeframe,
    )
...
def _build_full_window_backtest_plan(
    ...
    """Round 118 thin re-export — see :func:`backtest_engine.build_full_window_backtest_plan`."""

    return backtest_engine.build_full_window_backtest_plan(data_range, timeframe)


def _resolve_full_window_recommendation(context: Dict[str, Any]) -> Dict[str, Any]:
    """Round 118 thin re-export — see :func:`backtest_engine.resolve_full_window_recommendation`."""

    return backtest_engine.resolve_full_window_recommendation(context)
...
def _render_sample_validation_action(
    ...
    """Round 118 thin re-export — see :func:`backtest_engine.render_sample_validation_action`."""

    return backtest_engine.render_sample_validation_action(
        current_range,
        current_timeframe,
        payload,
    )
...
def _resolve_backtest_decision_readiness(context: Dict[str, Any]) -> Dict[str, Optional[str]]:
    """Round 118 thin re-export — see :func:`backtest_engine.resolve_backtest_decision_readiness`."""

    return backtest_engine.resolve_backtest_decision_readiness(context)
...
def _filter_sample_quality_guarded_parsed_proposals(
    ...
    """Round 118 thin re-export — see :func:`backtest_engine.filter_sample_quality_guarded_proposals`."""

    return backtest_engine.filter_sample_quality_guarded_proposals(proposals, context)
...
def _extract_safe_sample_validation_payload(
    ...
    """Round 118 thin re-export — see :func:`backtest_engine.extract_safe_sample_validation_payload`."""

    return backtest_engine.extract_safe_sample_validation_payload(payload, context)
...
def _is_reference_only_backtest_context(context: Dict[str, Any]) -> bool:
    """Round 118 thin re-export — see :func:`backtest_engine.is_reference_only_backtest_context`."""

    return backtest_engine.is_reference_only_backtest_context(context)


def _get_backtest_trade_count(context: Dict[str, Any]) -> int:
    """Round 118 thin re-export — see :func:`backtest_engine.get_backtest_trade_count`."""

    return backtest_engine.get_backtest_trade_count(context)


def _is_low_sample_backtest_context(context: Dict[str, Any]) -> bool:
    """Round 118 thin re-export — see :func:`backtest_engine.is_low_sample_backtest_context`."""

    return backtest_engine.is_low_sample_backtest_context(context)


def _is_truncated_backtest_context(context: Dict[str, Any]) -> bool:
    """Round 118 thin re-export — see :func:`backtest_engine.is_truncated_backtest_context`."""

    return backtest_engine.is_truncated_backtest_context(context)


def _is_market_detail_fallback_backtest_context(context: Dict[str, Any]) -> bool:
    """Round 118 thin re-export — see :func:`backtest_engine.is_market_detail_fallback_backtest_context`."""

    return backtest_engine.is_market_detail_fallback_backtest_context(context)


def _get_backtest_history_source_reason(context: Dict[str, Any]) -> str:
    """Round 118 thin re-export — see :func:`backtest_engine.get_backtest_history_source_reason`."""

    return backtest_engine.get_backtest_history_source_reason(context)


def _merge_backtest_review_risks(risks: List[str], context: Dict[str, Any]) -> List[str]:
    """Round 118 thin re-export — see :func:`backtest_engine.merge_backtest_review_risks`.

    The engine helper takes the execution-health risk merger as a callback so
    it doesn't have to import this module's ``_merge_execution_health_review_risks``
    wrapper directly; passing the wrapper here preserves the historical
    behaviour of routing through ``main``-side review-health enrichment.
    """

    return backtest_engine.merge_backtest_review_risks(
        risks,
        context,
        merge_execution_health_review_risks=_merge_execution_health_review_risks,
    )
...
def _clear_strategy_auto_dispatch_alerts(strategy_id: str) -> bool:
    return _clear_strategy_auto_dispatch_alerts_impl(strategy_id, repo=repo)

def _has_active_strategy_auto_dispatch_alert(strategy_id: str) -> bool:
    return _has_active_strategy_auto_dispatch_alert_impl(strategy_id, repo=repo)

def _clear_strategy_live_stop_loss_alerts(strategy_id: str) -> bool:
    return _clear_strategy_live_stop_loss_alerts_impl(strategy_id, repo=repo)

def _has_active_strategy_live_stop_loss_alert(strategy_id: str) -> bool:
    return _has_active_strategy_live_stop_loss_alert_impl(strategy_id, repo=repo)
...
def _clear_strategy_position_drift_alerts(
    ...
    *,
    ...
    return _clear_strategy_position_drift_alerts_impl(
        strategy_id,
        repo=repo,
...
def _has_active_strategy_position_drift_alert(strategy_id: str) -> bool:
    return _has_active_strategy_position_drift_alert_impl(strategy_id, repo=repo)
...
def _clear_strategy_exchange_rejected_alerts(
    ...
    *,
    ...
    return _clear_strategy_exchange_rejected_alerts_impl(
        strategy_id,
        repo=repo,
        resolve_strategy_parameter_snapshot=_resolve_strategy_parameter_snapshot,
        resolution_detail=resolution_detail,
...
def _clear_strategy_exchange_rejection_guard_alerts(
    ...
    *,
    ...
    return _clear_strategy_exchange_rejection_guard_alerts_impl(
        strategy_id,
        repo=repo,
        resolve_strategy_parameter_snapshot=_resolve_strategy_parameter_snapshot,
        resolution_detail=resolution_detail,
...
def _has_active_strategy_exchange_rejection_guard_alert(strategy_id: str) -> bool:
    return _has_active_strategy_exchange_rejection_guard_alert_impl(strategy_id, repo=repo)
...
def _clear_strategy_stale_order_alerts(
    ...
    *,
    ...
    return _clear_strategy_stale_order_alerts_impl(
        strategy_id,
        repo=repo,
        resolve_strategy_parameter_snapshot=_resolve_strategy_parameter_snapshot,
        resolution_detail=resolution_detail,
...
def _has_active_strategy_stale_order_alert(strategy_id: str) -> bool:
    return _has_active_strategy_stale_order_alert_impl(strategy_id, repo=repo)
...
def _queue_strategy_issue_review_locked(
    *,
    ...
    _queue_strategy_issue_review_locked_impl(
        repo=repo,
        strategy_id=strategy_id,
        strategy_name=strategy_name,
        symbol=symbol,
...
def _classify_strategy_auto_dispatch_alert_kind(detail: Optional[str]) -> Optional[str]:
    return _classify_strategy_auto_dispatch_alert_kind_impl(detail)


def _resolve_alert_sub_block_code(alert: Optional[AlertRecord]) -> Optional[str]:
    return _resolve_alert_sub_block_code_impl(alert)


def _derive_sub_block_code_from_detail(detail: Optional[str]) -> Optional[str]:
    return _derive_sub_block_code_from_detail_impl(detail)
...
def _record_strategy_auto_dispatch_issue(
    ...
    *,
    ...
    _record_strategy_auto_dispatch_issue_impl(
        strategy_id,
        strategy_name,
        symbol,
        signal,
        ...
        recommended_action,
        repo=repo,
        build_auto_dispatch_recommended_action=_build_auto_dispatch_recommended_action,
        strategy=strategy,
...
def _record_strategy_manual_execution_issue(
    ...
    *,
    ...
    _record_strategy_manual_execution_issue_impl(
        strategy_id,
        strategy_name,
        symbol,
        mode,
        ...
        recommended_action,
        repo=repo,
        build_manual_execution_recommended_action=_build_manual_execution_recommended_action,
        strategy=strategy,
        reason_code=reason_code,
        market=market,
        sub_block_code=sub_block_code,
...
def _clear_strategy_manual_execution_alerts(
    ...
    *,
    ...
    return _clear_strategy_manual_execution_alerts_impl(
        strategy_id,
        mode,
        repo=repo,
        resolve_strategy_parameter_snapshot=_resolve_strategy_parameter_snapshot,
...
def _sync_strategy_position_drift_issue(
    ...
    *,
    ...
    _sync_strategy_position_drift_issue_impl(
        snapshot,
...
def _record_strategy_live_stop_loss_issue(
    ...
    *,
    ...
    _record_strategy_live_stop_loss_issue_impl(
        strategy,
        snapshot,
        position,
